...
def parse_and_embed_data(dataset, test_size):
    examples = []
    ...
    for line in dataset:
        ...
        examples.append({"text": question, "label": answer})

    # create dataset using Hugging Face's Dataset class
    from datasets import Dataset
    dataset = Dataset.from_pandas(pd.DataFrame(examples), split="train")
...
synthetic_dataset = eval_pipe.synthetic_dataset
real_dataset = eval_pipe.real_dataset
synthetic_data = parse_and_embed_data(synthetic_dataset, test_size=0.0)
print(f"Synthetic data train length = {len(synthetic_data['train'])}, test length = {len(synthetic_data['test'])}")
real_data = parse_and_embed_data(real_dataset, test_size=0.2)
print(f"Real data train length = {len(real_data['train'])}, test length = {len(real_data['test'])}")

print(synthetic_data['train'][0])
...
def formatting_prompts_func(examples):
    ...
    for i in range(len(examples['prompt'])):
        text = f"### Instruction: {examples['text'][i]}\n ### Answer: {examples['label'][i]}"
